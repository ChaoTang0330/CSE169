...
bool Window::initializeProgram() {
	...
	// Create a shader program with a vertex shader and a fragment shader.
	skinShaderProgram = LoadShaders("shaders/shader_skin.vert", "shaders/shader.frag");

	// Check the shader program.
	if (!skinShaderProgram)
	{
		std::cerr << "Failed to initialize skin shader program" << std::endl;
		return false;
	}
...
void Window::idleCallback()
{
	...
	//cube->update();
	if (skel != nullptr)skel->update(glm::mat4(1.0));
...
void Window::plotImGUI()
{
	...
	for (int i = 0; i < numOfJoint; i++)
	{
		if (ImGui::TreeNode(jointsName[i].c_str()))
		{
			//ImGui::Text("RotX");
			//ImGui::SameLine();
			if (ImGui::DragFloat("RotX", &jointsRot[i].x,
				0.005f,
				jointsRotMin[i].x,
				jointsRotMax[i].x,
				"%.3f",
			...
			{
				skel->setJointRot(i, jointsRot[i]);
			}

			//ImGui::Text("RotY");
			//ImGui::SameLine();
			if (ImGui::DragFloat("RotY", &jointsRot[i].y,
				0.005f,
				jointsRotMin[i].y,
				jointsRotMax[i].y,
				"%.3f",
			...
			//ImGui::Text("RotZ");
			//ImGui::SameLine();
			if (ImGui::DragFloat("RotZ", &jointsRot[i].z,
				0.005f,
				jointsRotMin[i].z,
				jointsRotMax[i].z,
				"%.3f",
